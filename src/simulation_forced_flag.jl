...
const MIN_SEED_LEN = 6   # minimum nodes a restart must grow, else we roll it back
...
# Force a reseed far from the current cluster, then immediately grow K nodes.
# Returns: seed_count (>0 if success), and the list of indices added in this manual op.
function manual_reseed_and_prime!(
    state, config;
    L::Int = max(6, state.last_to_check),  # target mini-strand size
    K::Int = 8,                            # extra local growth attempts
    dmin::Float64 = 2.0,                   # distance window for seeding
    dmax::Float64 = 4.0
)
    # 1) try a full reseed that produces at least MIN_SEED_LEN nodes
    seed_cnt = seed_new_strand_far!(
        state, config;
        L = L,
        dmin = dmin, dmax = dmax,
        tries = 1000,
    ...
    if seed_cnt == 0
        @warn "manual_reseed_and_prime!: reseed failed to form a viable mini-strand."
        return 0, Int[]
    end

    # 2) hand off frontier to the new strand and reset placement state
    state.last_to_check = seed_cnt
    local_boix = 1

    # Find the first index of the just-added strand (we assume last `seed_cnt` nodes)
    n = length(state.x_coords)
    first_idx = n - seed_cnt + 1
    added = collect(first_idx:n)

    # 3) Prime further with K quick local growth steps
    successes = 0
    for _ in 1:K
        rs = add_monomer(state, config, local_boix)
        ok  = rs[1]
        local_boix = rs[2]
        if ok
            push!(added, length(state.x_coords))
            successes += 1
            # keep frontier near the new strand
            state.last_to_check = min(state.last_to_check + 1, length(added))
        else
            break
        end
    end

    @info "Manual reseed done" seed_cnt success_prime=successes new_total=length(state.x_coords)
    return seed_cnt, added
end

# If you already have a specific seed index and just want to grow locally around it:
function manual_prime_at!(state, config, seed_idx::Int; K::Int=8)
    local_boix = 1
    added = Int[]
    state.last_to_check = 1    # keep growth tightly near the newest nodes
    for _ in 1:K
        rs = add_monomer(state, config, local_boix)
        ok  = rs[1]
        local_boix = rs[2]
        if ok
            push!(added, length(state.x_coords))
            state.last_to_check = min(state.last_to_check + 1, K)
        else
            break
        end
    end
    @info "Manual prime-at done" seed_idx grew=length(added)
    return added
end
...
"""
Rollback the last `k` monomers and all edges incident to them.
Assumes those k monomers are the most recently appended (which is true during reseed).
"""
function rollback_last_k!(state, config, k::Int)
    k <= 0 && return
    n0 = length(state.x_coords)
    n1 = n0 - k + 1                     # first index to remove

    # remove edges that reference any node >= n1
    keep = Vector{Tuple{Int,Int}}()
    sizehint!(keep, length(state.edges))
    @inbounds for (u,v) in state.edges
        if u < n1 && v < n1
            push!(keep, (u,v))
        end
    end
    ...
    # shrink degree to n1-1
    if length(state.degree) >= n1
        resize!(state.degree, n1 - 1)
    end

    # remove nodes (and update box structures)
    for idx in n0:-1:n1
        x = state.x_coords[idx]; y = state.y_coords[idx]
        update_box_list!(state.boxList, state.boxNum, x, y, idx, :remove, config)
        pop!(state.x_coords); pop!(state.y_coords)
        pop!(state.NN)
        pop!(state.rotation)
    end
end
...
function seed_new_strand_far!(
    state, config;
    L::Int = state.last_to_check,    # target length for the new strand
    dmin::Float64 = 100.0,
    dmax::Float64 = 500.0,
    ...
    seed_idx = seed_new_start_far!(state, config; dmin=dmin, dmax=dmax, tries=tries)
    seed_idx == 0 && return 0

    added = Int[seed_idx]                   # indices we added in this reseed
    placed = 1
    local_window = 1

    # 2) grow up to L-1 more nodes anchored on this new seed cluster only
    for _ in 2:L
        state.last_to_check = local_window  # restrict available_space to the newest nodes
        local_fails = 0
        while true
            rs = add_monomer(state, config, 1)  # boix value unused here
            ok  = rs[1]
            if ok
                placed += 1
                push!(added, length(state.x_coords))  # newest index
                local_window = min(local_window + 1, L)
                break
            else
                local_fails += 1
                if local_fails >= per_node_fail
                    # couldn't place this node; stop extending
                    break
                end
            end
        end
        # stop outer loop as well if we failed to place this step
        if local_fails >= per_node_fail; break; end
    end

    # 3) require a minimum strand size; otherwise rollback the entire reseed
    if placed < MIN_SEED_LEN
        rollback_last_k!(state, config, length(added))
        return 0
    end

    # 4) success: hand off the frontier to this new strand (critical to avoid islands)
    state.last_to_check = placed
    return placed
end
